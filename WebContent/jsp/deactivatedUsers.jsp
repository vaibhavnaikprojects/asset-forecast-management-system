					...
					<div class="col-md-12 col-sm-12 col-xs-12">
						<h4 style="color: #336699">
							<b><c:if test="${not empty employeeBean}">${employeeBean.employeeName}</c:if>
								${success} <span style="color: red;">${message}</span> </b>
						</h4>
								...
								<tbody>
									<c:forEach var="employee"
										...
										<c:if test="${employee.status ne 'A'}">
											<tr data-toggle="modal" data-id="${employee.userId}"
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        	...
        	<form id="activateEmployee" method="post" action="activateUser.html">
      			<input type="hidden" class="form-control" name="userId" id="userId"/>
      			<input type="hidden" class="form-control" name="employeeName" id="employeeName"/>
      			<input type="hidden" class="form-control" name="ogdesignation" id="ogdesignation"/>
      			<c:if test="${sessionScope.employee.designation eq 'PCO Team'}">
      				<button type="submit" class="btn btn-success " style="position: absolute; top:92.5%;left:3%;">Activate User</button>
      			</c:if>
      		</form>
      		</div>
      </div>

    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
				<div class="form-group">
					...
					<input type="number" class="form-control" name="currentHeadCount" id="currentHeadCount" required=""/>
					<label for="growthCount">Growth/Decline Count</label>
					<input type="number" class="form-control" name="growthCount" id="growthCount" required=""/>
					...
					<option value="" disabled selected>Select Project Manager</option>
						<c:forEach var="programManagers" items="${sessionScope.employee.employeeBeans}">
					...
					<input type="text" class="form-control" name="deliveryHead" id="deliveryHead" value="${sessionScope.employee.employeeName}" required="required" readonly="readonly"/>
        			</div>
	  </div>
...
<!-- Modal For Sending Report To ODC Head -->
<div class="modal fade" id="sendReport" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Send Report To CISCO ODC HEAD</h4>
      </div>
      <form action="sendReport.html" method="post" class="form-inline">
      <div class="modal-body">
				<div class="form-group">
					<label for="exportYear">Enter Year</label>
					<input class="form-control" type="text" name="reportYear" id="reportYear" required=""/>
					<label for="reportQuarter">Select Quarter</label>
						<select class="form-control" id="reportQuarter" name="reportQuarter" required="">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
     ...
     function selectall()
     {
    	 ...
    	 var x = unique.programManager.selectedIndex;
    	    var y = unique.programManager.options;
    	    if(y[x].text=="All")
    	    	{
      ...
      <div class="modal-body">
				<div class="form-group">
					...
					<select class="form-control" id="programManager" name="programManager" required="" onchange="javaScript:selectall()">
						<option value="" disabled selected>Select Program Manager</option>
						<c:forEach var="employee" items="${sessionScope.employee.employeeBeans}">
							<option value="${employee.userId}">${employee.employeeName}</option>
						</c:forEach>
					...
					<select class="form-control" id="projectManager" name="projectManager" required="">
						...
						<c:forEach var="emp" items="${sessionScope.employee.employeeBeans}">
							<c:forEach var="employee" items="${emp.employeeBeans}">
							<option value="${employee.userId}">${employee.employeeName}</option>
							</c:forEach>
						</c:forEach>
						<option value="ALL">All</option>
...
<div class="panel panel-default">
    <div class="panel-heading" role="tab" id="${programManagers.employeeName}">
      <h4 class="panel-title" style="color: #336699">
        <a role="button" data-toggle="collapse" data-parent="#accordion" href="#${programManagers.userId}" aria-expanded="true" aria-controls="${programManagers.employeeName}">
    ...
    <div id="${programManagers.userId}" class="panel-collapse collapse" role="tabpanel" aria-labelledby="${programManagers.employeeName}">
    	...
    	<c:forEach var="programManagers" items="${sessionScope.employee.employeeBeans}">
				<c:forEach var="projectManagers" items="${programManagers.employeeBeans}">
					<div class="panel panel-default">
						<div class="panel-heading" role="tab" id="${projectManagers.employeeName}">
		     				<h4 class="panel-title" style="color: #336699">
		        			<a role="button" data-toggle="collapse" data-parent="#accordion2" href="#${fn:replace(projectManagers.employeeName,' ', '')}" aria-expanded="true" aria-controls="${projectManagers.employeeName}" onclick="return test1('${projectManagers.userId}')">
		        			${projectManagers.employeeName}
		        			</a>

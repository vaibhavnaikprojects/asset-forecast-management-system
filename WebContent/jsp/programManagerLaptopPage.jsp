...
function editRequest(laptopId)
{
	
	
	$.ajax({
		dataType: "json",
		url : "getLaptop.html?laptopId=" + laptopId,
		type : "GET",
		success : function(data) {
			if (data !=null) {
				editLaptopRequest.laptopId.value=data.laptopId;
				editLaptopRequest.ownerName.value=data.ownerName;
				editLaptopRequest.requestType.value=data.requestType;
				editLaptopRequest.previousReason.value=data.previousReason;
				editLaptopRequest.createdDate.value=data.createdDate;
				editLaptopRequest.projectName.value=data.projectName;
				//editLaptopRequest.createdDate.value=data.createdDate;
			} else {
...
<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
		    ...
		    <div id="collapseOne" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
		      <div class="panel-body">
		        	<div class="form-group">
		        			<form id="requestNewHireRequest" action="raiseNewHireLaptopRequest.html" method="post">
		        			<label for="projectName">From : </label>
							<select class="form-control" id="fromType" name="fromType" required="" onchange="showFromDiv(this)">
		...
		<input type="text" class="form-control-half-name" name="fromAssociate" id="fromAssociate"  onblur="return frommanagerNameH()" placeholder="From : CEC ID" style="display: none"/>
		
		<input type="text" class="form-control-half-name" name="fromAssociateName" id="fromAssociateName"  placeholder="From Associate NAME" readonly="readonly" style="display: none"/>
			<div class="form-control-clear"></div>
		        			
		        			<br>
							
							<input type="text" class="form-control-half-name" name="toAssociate" id="toAssociate" onblur="return managerNameH()"  placeholder="To : CEC ID" required="required"/>
							
							<input type="text" class="form-control-half-name" name="toAssociateName" id="toAssociateName"  placeholder="To Associate NAME" readonly="readonly" required="required"/>
							<div class="form-control-clear"></div></br>
							<label for="reason">Reason</label>
							<textarea class="form-control" name="reason" id="reason" maxlength="2000"
								required="required"></textarea>
							<label for="projectName">Project Name</label>
							<select class="form-control" id="projectName" name="projectName" required="" title="Select Project Name">
								...
								<c:forEach var="asset1" items="${sessionScope.employee.employeeBeans}">
							...
							<input type="hidden" class="form-control" name="requestType" id="requestType" value="1"/>
							<br/>
							<button type="submit" class="btn btn-primary"    onclick="return checkDoller();return Submit()" >Raise Request</button>
		  ...
		  <div class="panel panel-default">
		    ...
		    <div id="collapseTwo" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingTwo">
		      <div class="panel-body">
		       <div class="form-group">
		        			<form id="requestEarlyRefreshRequest" action="raiseEarlyRefreshLaptopRequest.html" method="post" enctype="multipart/form-data">
		        			<label for="projectName">From : </label>
							<select class="form-control" id="fromType1" name="fromType1" required="" onchange="showFromDiv1()">
								<option value="" disabled selected>Select</option>
								<option value="N">Associate (Transfer Request)</option>
								<option value="S">Stock</option>
								</select>
		...
		<input type="text" class="form-control-half-name" name="fromAssociate" id="fromAssociate"  onblur="return frommanagerName()" placeholder="From : CEC ID" style="display: none"/>
		<input type="text" class="form-control-half-name" name="fromAssociateName" id="fromAssociateName"  placeholder="From : Associate NAME" style="display: none"/>
		
			<div class="form-control-clear"></div>
		<br>        			
							<!-- <label for="toAssociate">To Associate</label> -->
							<input type="text" class="form-control-half-name" name="toAssociate" id="toAssociate" onblur="return managerName()"  placeholder="To : CEC ID" />
							<input type="text" class="form-control-half-name" name="toAssociateName" id="toAssociateName"  placeholder="To : Associate NAME" readonly="readonly"/>
							<div class="form-control-clear"></div></br>
							...
							<label for="ciscoManagerMail">Cisco Manager Approval Mail</label>
							<input type="file"  id="ciscoManagerMail" name="ciscoManagerMail" required="" onchange="alertFilename()"/>
							<input type="hidden" class="form-control" name="requestType" id="requestType" value="2"/>
							<br/>
							<button type="submit" class="btn btn-primary"   onclick="return checkDoller();return Submit()"   >Raise Request</button>
		  ...
		  <div class="panel panel-default">
		    <div class="panel-heading" role="tab" id="headingThree">
		     ...
		     <div id="collapseThree" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingThree">
		      <div class="panel-body">
		        <c:if test="${not empty sessionScope.employee.laptopBeans}">
		        <div class="x_panel">
					<table id="laptopRequests" class="table table-hover">
								...
								<tbody>
									...
									<c:choose>
										<c:when test="${laptop.statusFlagValue=='Approved'}">
											<td><i class="fa fa-check fa-lg" style="color: #1ABB9C"
												title="Request approved "></i></td>
										</c:when>
										<c:when test="${laptop.statusFlagValue=='Reject'}">
											<td><i class="fa fa-times fa-lg" style="color: #1ABB9C"
												title="Request rejected "></i></td>
										</c:when>
				...
				<c:if test="${empty sessionScope.employee.laptopBeans}">No Requests Yet</c:if>
		     </div>
		     </div>
		  </div>
		</div>
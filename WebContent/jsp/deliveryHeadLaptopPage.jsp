...
<script type="text/javascript">
	...
	function showFromDivEarly() {

		var a = $("#fromTypeEarly :selected").text();
		if (a == "Associate (Transfer Request)") {
			$('label[name=fromAssociateEarly]').show();
			$('input[name=fromAssociateEarly]').show();
			$('input[name=fromAssociateNameEarly]').show();
		} else {
			$('label[name=fromAssociateEarly]').hide();
			$('input[name=fromAssociateEarly]').hide();
			$('input[name=fromAssociateNameEarly]').hide();
		...
		<div class="panel-heading" role="tab" id="headingOne">
			<h4 class="panel-title" style="color: #336699">
				<a role="button" data-toggle="collapse" data-parent="#accordion"
					...
					aria-controls="collapseOne"> Raise New Hire Laptop Request </a>
			</h4>
		</div>
				...
				<div class="form-group">
					<form id="requestNewHireRequest"
						...
						<label for="projectName">From : </label> <select
							class="form-control" id="fromType" name="fromType" required=""
							onchange="showFromDiv(this)">
						...
						<br> <input type="text" class="form-control-half-name"
							...
							onblur="return managerNameH()" placeholder="To : CEC ID" /> <input
							type="text" class="form-control-half-name" name="toAssociateName"
						...
						<label for="projectName">Project Name</label> <select
							...
							<c:forEach var="asset1"
								...
								<c:forEach var="assetsdetails" items="${asset1.employeeBeans}">
									<c:forEach var="assetsdetails11"
										items="${asset1.employeeBeans}">
										<c:forEach var="assetsdetaildept"
											items="${assetsdetails11.assetBeans}">
											<option value="${assetsdetaildept.projectName}">${assetsdetaildept.projectName}</option>
										</c:forEach>
									</c:forEach>
								</c:forEach>
							</c:forEach>
			...
			<div class="panel-body">
				<div class="form-group">
					<form id="requestEarlyRefreshRequest"
						...
						<label for="projectName">From : </label> <select
							class="form-control" id="fromTypeEarly" name="fromTypeEarly"
							required="" onchange="showFromDivEarly(this)">
							<option value="" disabled selected>Select</option>
							<option value="N">Associate (Transfer Request)</option>
							<option value="S">Stock</option>
						</select> <br>
						...
						<input type="text" class="form-control-half-name"
							name="fromAssociateEarly" id="fromAssociateEarly"
							onblur="return frommanagerName()" placeholder="From : CEC ID"
							style="display: none" /> <input type="text"
							class="form-control-half-name" name="fromAssociateNameEarly"
							id="fromAssociateNameEarly" placeholder="From : Associate NAME"
						...
						<input type="text" class="form-control-half-name"
							name="toAssociate" id="toAssociate" onblur="return managerName()"
							placeholder="To : CEC ID" /> <input type="text"
						...
						</select> <label for="ciscoManagerMail">Cisco Manager Approval Mail</label>
						<input type="file" id="ciscoManagerMail" name="ciscoManagerMail"
							required="" onchange="alertFilename()" /> <input type="hidden"
										...
										<c:choose>
											...
											<c:otherwise>
												<td><a
													...
													onclick="return warningSubmit()"> <i
